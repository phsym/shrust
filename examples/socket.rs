...
fn main() {
    ...
    shell.new_command(&"put", &"Insert a value", 2, |_, map, args| {
        map.lock().unwrap().insert(r#try!(usize::from_str(args[0])), args[1].to_string());
        Ok(())
    });
    shell.new_command("get", "Get a value", 1, |io, map, args| {
        match map.lock().unwrap().get(&r#try!(usize::from_str(args[0]))) {
    ...
    shell.new_command("remove", "Remove a value", 1, |_, map, args| {
        map.lock().unwrap().remove(&r#try!(usize::from_str(args[0])));
        Ok(())
    });
